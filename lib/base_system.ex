...
defmodule BaseSystem.Configure do
	...
	def configure(tags, opts) do
		...
		port_forwards =
			tags
			|> Enum.map(&parse_port_forward_tag/1)
			...
			%RedoAfterMeet{
				...
				unit:   %All{units: [
					...
					%FilePresent{
						...
						content: EEx.eval_string(content("files/etc/chrony/chrony.conf.eex")),
						mode:    0o644
					},
				]},
	...
	defp parse_port_forward_tag(tag) do
		case Regex.run(~r/\Aport_forward:(.+);(.+);(.+);(.+);(.+)\z/, tag, capture: :all_but_first) do
			nil -> nil
			[protocol, source_ip, source_port, destination_ip, destination_port] ->
				%{
					protocol:         protocol,
					source_ip:        source_ip,
					source_port:      String.to_integer(source_port),
					destination_ip:   destination_ip,
					destination_port: String.to_integer(destination_port)
				}
		end
	end
	...
	def make_ferm_config(verbose_log, input_chain, output_chain, forward_chain, postrouting_chain, ssh_port, wireguard_port, port_forwards) do
		...
		#{port_forwards |> Enum.map(&port_forward_accept_line/1) |> Enum.join("\n") |> indent |> indent}
		...
		#{postrouting_chain |> Enum.join("\n") |> indent |> indent}
			...
			# PREROUTING for traffic coming from other machines
			# OUTPUT for traffic generated by host machine
			chain (PREROUTING OUTPUT) {
		#{port_forwards |> Enum.map(&port_forward_dnat_line/1) |> Enum.join("\n") |> indent |> indent}
			}
		}
		"""
	end

	defp port_forward_accept_line(f) do
		"proto #{f.proto} daddr #{f.destination_ip} dport #{f.destination_port} ACCEPT;"
	end

	defp port_forward_dnat_line(f) do
		"proto #{f.proto} daddr #{f.source_ip} dport #{f.source_port} DNAT to #{f.destination_ip}:#{f.destination_port};"
	end
